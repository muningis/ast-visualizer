...
export function NodeCard(props: NodeCardProps) {
 ...
 return `<article class="w-[150px] min-h-[60px] border-2 ${borderColorByAstType[astGroupByType[props.type] as AstGroup]} bg-white text-center">
 <h2 class="p-2 ${headerColorByAstType[astGroupByType[props.type] as AstGroup]} border-b-2 ${borderColorByAstType[astGroupByType[props.type] as AstGroup]}" >${props.type}</h2>
 <div class="p-2">
 <h3>${props.content ?? ""}</h3>
  ${/* ${name}
    ${value}
